...
//	auto result = con.Query("select fuck('Jane') as result");
	auto result = con.Query("select n_name from '/scratch/liyu/opt/pixels-reader-cxx/tests/data/nation_0_01.pxl'");
	result->Print()
	auto result1 = con.Query("select * from '/scratch/liyu/opt/pixels-reader-cxx/tests/data/nation_0_01.pxl'");
	result1->Print();
}
